...
import cn.superion.materialDept.list.rejectList.view.WinRejectListQuery;
...
public const DESTINATION:String='deptRejectListImpl';
...
{
	this.parentDocument.title="报损单据列表";
	initPanel();
	initToolBar();
}
...
{	
	...
	gdRejectList.grid.horizontalScrollPolicy="on";
...
{
	var dataList:ArrayCollection=gdRejectList.grid.dataProvider as ArrayCollection;
	...
	if (printSign == '1')
	{
		ReportPrinter.LoadAndPrint("report/materialDept/list/rejectList.xml", dataList, dict);		
	}
	else
	{
		ReportViewer.Instance.Show("report/materialDept/list/rejectList.xml", dataList, dict);		
	}
}
...
{
	var queryWin:WinRejectListQuery=PopUpManager.createPopUp(this, WinRejectListQuery, true) as WinRejectListQuery;
 ...
 * 部门
 */
private function deptLBF(item:Object, column:DataGridColumn):String
{
	if (item.deptCode == '')
	{
		item.deptName='';
	...
	{
		var dept:Object=ArrayCollUtils.findItemInArrayByValue(BaseDict.deptDict, 'dept', item.outDeptCode);	
		item.deptName=dept == null ? "" : dept.deptName;
	}
