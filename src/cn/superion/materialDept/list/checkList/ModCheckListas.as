...
import cn.superion.materialDept.list.checkList.view.WinCheckListQuery;
...
{
	this.parentDocument.title="盘点单据列表";
	initPanel();
	initToolBar();
}
...
{	
	dgCheckList.grid.sumRowLabelText='合计';
	dgCheckList.grid.horizontalScrollPolicy="on";
	var paramQuery:ParameterObject=new ParameterObject();
	paramQuery.conditions={};
	dgCheckList.config(paramQuery, DESTINATION, 'findCheckDetailListByCondition', function(rev:Object):void
	{
		setToolBarBts(rev);		
		for (var i:int;i<rev.data.length;i++)
		{
		    ...
		    //账面金额
			item.counterMoney=item.amount * item.retailPrice;
			//盘点金额
			item.pdMoney=item.retailPrice * item.checkAmount;
			//盈亏数量
			item.profitAmount=item.amount - item.checkAmount;
			//盈亏金额
			item.profitCost=item.retailPrice * item.profitAmount;
...
{
	var dataList:ArrayCollection=dgCheckList.grid.dataProvider as ArrayCollection;
	...
	if (printSign == '1')
	{
		ReportPrinter.LoadAndPrint("report/materialDept/list/checkList.xml", dataList, dict);
		
	}
	else
	{
		ReportViewer.Instance.Show("report/materialDept/list/checkList.xml", dataList, dict);
...
{
	var queryWin:WinCheckListQuery=PopUpManager.createPopUp(this, WinCheckListQuery, true) as WinCheckListQuery;
