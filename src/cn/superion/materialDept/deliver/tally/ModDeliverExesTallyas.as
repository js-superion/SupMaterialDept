...
protected function batch_queryIconClickHandler():void
{
	if (dataGridDetail.selectedItem)
	{
		...
		MaterialDictShower.showBatchNoChooser(dataGridDetail.selectedItem.materialId, "", null, function(rev:Object):void
			{
				if (dataGridDetail.selectedItem)
				{
					...
					dataGridDetail.selectedItem.madeDate=rev.madeDate;
					dataGridDetail.selectedItem.factoryCode=rev.factoryCode;
					dataGridDetail.selectedItem.factoryName=rev.factoryName;
					dataGridDetail.selectedItem.tradePrice=rev.tradePrice;
					dataGridDetail.selectedItem.tradeMoney=rev.tradePrice;
					dataGridDetail.selectedItem.wholeSalePrice=rev.wholeSalePrice;
					dataGridDetail.selectedItem.wholeSaleMoney=rev.wholeSalePrice;
					dataGridDetail.selectedItem.retailPrice=rev.retailPrice;
				}
...
//	MaterialDictShower.showMaterialDictNew(lstorageCode, '', '', true, function(faryItems:Object):void
//	{
//		fillIntoGrid(faryItems);
//	}, x, y,isZeroDeliver);
}
...
private function fillIntoGrid(fItem:Object):void
{
	...
	for(var i:int=0;i<fItem.length;i++){
		...
		materialPatsDetail=fillDetailForm(fItem[i]);
